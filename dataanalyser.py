...
def populateDict(grids, fileString, groupType):
    for i in range(1, len(fileString)):
        ...
        if (currentLine[0] == "0"):
            isLookedAt = [0] * len(grids)                 #List to check if a grid is already looked at by current user.
        if (isLookedAt[index-1] == 0):                    #Increase no of people
            grids[index][groupType]["noOfPeople"] += 1
            isLookedAt[index-1] = 1                       #Marks grid as checked by current user.
...
# Populate dictionary with grid information
